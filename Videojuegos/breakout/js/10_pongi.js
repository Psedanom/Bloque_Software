...
class Ball extends GameObject {
    ...
    serve() {
        let angle = Math.random() * 2 * (Math.PI / 2) - (Math.PI / 4);
...
class Game {
    ...
    initObjects() {
        ...
        this.pointsTextLeft = new TextLabel(canvasWidth / 4, 80, "20px Arial", "black");
        this.pointsTextRight = new TextLabel(canvasWidth / 4*3, 80, "20px Arial", "black");
    ...
    draw(ctx) {
        ...
        // this.pointsTextLeft.draw(ctx, `Player 1: ${this.pointsLeft}`);
    ...
    update(deltaTime) {
        ...
        if (boxOverlap(this.ball, this.barrierTop) || boxOverlap(this.ball, this.barrierBottom)) {
            console.log("Collided with barrier");
        ...
        if (boxOverlap(this.ball, this.goalRight) ) {
            this.pointsLeft += 1;MathMLElement;
            this.ball.reset();
